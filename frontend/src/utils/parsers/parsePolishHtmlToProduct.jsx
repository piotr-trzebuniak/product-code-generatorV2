...
export function parsePolishHtmlToProduct(html) {
  ...
  if (skladSection) {
    // Wielkość opakowania
    const pSize = skladSection.nodes.find((n) => /Wielkość opakowania/i.test(n.textContent));
    if (pSize) {
      const strong = q("strong", pSize);
      const raw = strong ? getText(strong) : pSize.textContent.replace(/.*?:/, "");
      const { amount, unit } = parseAmountUnit(raw);
      sizeAmount = amount; sizeUnitPL = unit;
    }
